...
- **Dernière session** : Session 15 — machine abdos/lombaire sur les jours jambes + audit E2E complet (Playwright/web) avec 3 bugs réels trouvés et corrigés :
  - ✅ **Machine abdos + Extensions lombaires** — une par jour jambes (Extensions lombaires → Bas A, Machine abdos → Bas B) pour équilibrer le travail du tronc. `SEED_VERSION` 7→8.
  - ✅ **[BUG] Poids/reps perdus en complétant un set** — `handleUpdateSet('toggleComplete')` ne persistait que `completed_at`, sans jamais garantir que le poids/reps saisis avaient bien été écrits via `onEndEditing` du champ (pas fiable si le check est tapé juste après la saisie). Repéré en rechargeant la page après avoir complété un set : le set restait "fait" mais poids/reps revenaient à 0. Fix : flush des `pendingValues` dans le même payload que `toggleComplete`, comme le fait déjà `handleFinish`.
  - ✅ **[BUG] Sets ne se déplaçaient pas visuellement (Monter/Descendre)** — `grouped()` n'a jamais retrié `sets` par `set_index` après un déplacement (seul le tableau `session.sets` était muté), donc les lignes restaient à la même position, seul le numéro affiché changeait. Fix : tri par `set_index` dans `grouped()`.
  - ✅ **[BUG régression] Écran "Choisir un programme" cassé** — `constants/programs.ts` contenait 6 programmes prédéfinis (2 à 6 jours) tous construits sur les anciens templates Push/Pull/Legs/Upper/Lower supprimés en session 14 ; plus aucun jour ne pouvait matcher une routine réelle. Remplacé par un seul programme aligné sur Haut/Bas A+B. À surveiller : toujours vérifier les dépendances (`constants/`, pas seulement `DEFAULT_TEMPLATES`) avant de supprimer des templates par défaut.
  - ℹ️ Méthode : app lancée en web via Playwright + Chromium headless (`/opt/pw-browsers`), navigation réelle (remplir formulaires, taper les boutons, recharger la page pour vérifier la persistance SQLite) plutôt que lecture de code seule.
- **Session précédente (14)** : suppression des anciennes routines Push/Pull/Legs/Upper/Lower (confirmé par l'utilisateur) :
